...
video7.loop = true; // At the end, this becomes false and the video ends with the eyes closed covering the screen
...
p5 = new P5({width: window.innerWidth, height:window.innerHeight, mode: 'webgl'})
shader5 = p5.loadShader("https://raw.githubusercontent.com/aaronsherwood/liveCoding/main/Class_Examples/shaders/basic.vert", "/Users/nyuad/Downloads/shader_noiseSquareTunnel.frag");
p5.hide() //



//3. play with this before even introducing the character ~ 1 minute (Chenxuan)

osc(10, 0.4, 0)
.color(0, 0.2, 0)
.mult(noise(9, 1.2))
.modulate(voronoi(4, 0.5))
.modulateKaleid(osc(() => 2, 0.5), 3)
.kaleid(100)
.scrollX(1.2, -0.2)
.scrollY(1, 0.2)
.out(o8)
...
//5. get the first shader in (Chenxuan) - 7. Look for punches in the video and move one shader up every time there's a punch (Chenxuan)
...
//4. This triggers the video when o9 is rendered (Aadhar) - 6. Trigger the first video when Chenxuan starts with the shader (Aadhar) - 7. Play with voronoi, noise, colorama and scrollX (Aadhar and Chenxuan)

src(o3)
  .layer(src(o8).luma(0.3))
  .mult(voronoi(1, 0.5))
  .blend(shape(3, [0.04, 0.3].smooth()).scrollY(3, 0.3).scrollX(Math.random(-250), -0.5).luma(0.2,0.1), 0.2)
  .blend(shape(5, [0.1, 0.2].smooth()).scrollY(3, 0.3).scrollX(Math.random(200), 0.1), 0.2)
  .blend(shape(7, [0.01, 0.1].smooth()).scrollY(3, 0.3).scrollX(Math.random(-200), -0.1).luma(0.5,0.1), 0.2)
